...
{
    print_ch("解密后明文为:", de_mpt, real_length);
}
...
void DES::xor_s_swap(bool* RPT_s, const bool* pk_com_per, const bool* RPT_ex) {
    ...
    for (int i = 0; i < 8; i++) {
        int row = 0, col = 0;
        {
            row = temp[i * 6]<<1+ temp[6 * i + 5];
            col = (temp[6 * i + 1] <<3 )+ (temp[6 * i + 2]<<2)+(temp[6 * i + 3]<<1) + temp[6 * i + 4];
        }
        num = SSS[i][row][col];
