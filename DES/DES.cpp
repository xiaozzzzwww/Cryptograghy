...
//赋值
void DES::assign_val(bool* output, const bool* input, int length)
{
...
//置换
void DES::permutation(bool* output, const bool* input,int *p,int p_length) {
...
void DES::En_cryption() {
    ...
    permutation(mpt, temp, FP,64);
    assign_val(en_mpt, mpt,64);
    BitToByte(en_mpt_byte, mpt, 64);
}
...
void DES::De_cryption() {
    ...
    merge_L_RPT(temp, LPT, RPT);
    permutation(mpt, temp, FP,64);
    assign_val(de_mpt, mpt, 64);
    BitToByte(de_mpt_byte, mpt, 64);
}
